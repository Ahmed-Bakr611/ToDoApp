<div class="max-w-[900px] mx-auto p-5">
  @if (session()->has('success'))
    <div class="bg-green-100 border border-green-200 text-green-800 px-4 py-3 rounded-md mb-5 text-sm" x-data x-init="setTimeout(() => { $el.remove() }, 5000)">
      {{ session('success') }}
    </div>
  @endif

  @if (session()->has('error'))
    <div class="bg-red-100 border border-red-200 text-red-800 px-4 py-3 rounded-md mb-5 text-sm" x-data x-init="setTimeout(() => { $el.remove() }, 5000)">
      {{ session('error') }}
    </div>
  @endif
  ...
  @else
    <div class="text-center py-12">
      ...
      <p class="mt-1 text-sm text-gray-500">Tags will appear here once you create them while adding tasks.</p>
    </div>
  @endif
</div>
