<div class="max-w-6xl mx-auto p-5">
  ...
  @if (session()->has('success'))
    <div class="bg-green-100 border border-green-200 text-green-800 px-4 py-3 rounded-lg mb-5 text-sm" x-data x-init="setTimeout(() => { $el.remove() }, 5000)">
      {{ session('success') }}
    </div>
  @endif

  @if (session()->has('error'))
    <div class="bg-red-100 border border-red-200 text-red-800 px-4 py-3 rounded-lg mb-5 text-sm" x-data x-init="setTimeout(() => { $el.remove() }, 5000)">
      {{ session('error') }}
    </div>
  @endif
